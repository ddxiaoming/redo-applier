...
* Why use this malloc?
  ...
  * For large (>= 512 bytes) requests, it is a pure best-fit allocator,
    with ties normally decided via FIFO (_i.e. least recently used).
...
* Vital statistics:
  ...
  Alignment:                              2 * sizeof(size_t) (default)
       (_i.e., 8 byte alignment with 4byte size_t). This suffices for
  ...
  Minimum allocated size: 4-byte ptrs:  16 bytes    (including 4 overhead)
       ...
       Even a request for zero bytes (_i.e., malloc(0)) returns a
       pointer to something of the minimum allocatable size.

       The maximum overhead wastage (_i.e., number of extra bytes
...
#define MMAP_CLEARS 0
    ...
    /*
       ...
       this is known to be useful (_i.e. most linux kernels), this occurs
...
/*
  ...
  It has no effect if p is null. It can have arbitrary (_i.e., bad!)
...
/*
  ...
  corresponding parameter to the argument value if it can (_i.e., so
...
/*
  ...
  smblks:    the number of fastbin blocks (_i.e., small chunks that
...
/*
  ...
  The "chunks" argument is optional (_i.e., may be null, which is
...
/*
  ...
  The "chunks" argument is optional (_i.e., may be null). If it is null
...
#define inuse(p)                                                       \
    ((((mchunkptr)(((char*)(p)) + ((p)->size & ~PREV_INUSE)))->size) & \
     PREV_INUSE)
...
/*
  Top
    ...
    The top-most available chunk (_i.e., the one bordering the end of
...
#endif
{
    ...
    /*
      ...
      always true of any allocated chunk; _i.e., that each allocated
...
static void do_check_malloc_state(void)
{
    ...
    if (av->top == 0 || av->top == initial_top(av)) return;
    ...
    for (_i = 0; NFASTBINS - _i > 0; ++_i) {
        p = av->fastbins[_i];

        /* all bins past max_fast are empty */
        if (_i > max_fast_bin) assert(p == 0);
        ...
        while (p != 0) {
            ...
            assert(fastbin_index(chunksize(p)) == _i);
            p = p->fd;
        }
    }
    ...
    /* check normal bins */
    for (_i = 1; _i < NBINS; ++_i) {
        b = bin_at(av, _i);

        /* binmap is accurate (except for bin 1 == unsorted_chunks) */
        if (_i >= 2) {
            binbit = get_binmap(av, _i);
            empty = last(b) == b;
            if (!binbit)
                assert(empty);
            else if (!empty)
        ...
        for (p = last(b); p != b; p = p->bk) {
            ...
            if (_i >= 2) {
                ...
                assert(idx == _i);
                /* lists are sorted */
                if ((CHUNK_SIZE_T)size >=
                    (CHUNK_SIZE_T)(FIRST_SORTED_BIN_SIZE)) {
                    assert(p->bk == b || (CHUNK_SIZE_T)chunksize(p->bk) >=
    ...
    */
    ...
    if (brk != (char*)(MORECORE_FAILURE)) {
        ...
        */
            ...
            */
            ...
            if (contiguous(av)) {
                ...
                /*
                   ...
                   stats reporting.
                */
                if (old_size != 0) av->sbrked_mem += brk - old_end;
        ...
        /* Update statistics */
        sum = av->sbrked_mem;
        if (sum > (CHUNK_SIZE_T)(av->max_sbrked_mem)) av->max_sbrked_mem = sum;

        sum += av->mmapped_mem;
        if (sum > (CHUNK_SIZE_T)(av->max_total_mem)) av->max_total_mem = sum;
...
#endif
{
        ...
        if (in_smallbin_range(size)) {
            ...
            if (fwd != bck) {
                ...
                if ((CHUNK_SIZE_T)(size) < (CHUNK_SIZE_T)(bck->bk->size)) {
                    ...
                    size |=
                        PREV_INUSE; /* Or with inuse bit to speed comparisons */
    ...
    */
    ...
    /*
      ...
      bin. This search is strictly by best-fit; _i.e., the smallest
...
use_top:
    /*
      ...
      We require that av->top always exists (_i.e., has size >=
    ...
    */
    ...
    if ((((PTR_UINT)(m)) % alignment) != 0) { /* misaligned */
        ...
        */
        ...
        if ((CHUNK_SIZE_T)(brk - (char*)(p)) < MINSIZE) brk += alignment;
...
#if __STD_C
static Void_t** iALLOc(size_t n_elements, size_t* sizes, int opts,
                       Void_t* chunks[])
#else
...
#endif
{
    ...
    if (chunks != 0) {
        ...
        if (n_elements == 0) return (Void_t**)mALLOc(0);
        marray = 0;
   ...
   */
    ...
    if (marray != chunks) {
        ...
        if (element_size != 0)
            ...
            assert(remainder_size == request2size(sizes[_i]));
        check_inuse_chunk(mem2chunk(marray));
    }

    for (_i = 0; _i != n_elements; ++_i)
        check_inuse_chunk(mem2chunk(marray[_i]));
#endif
...
/*
  ...
  else (_i.e. If MORECORE_CONTIGUOUS is true):
...
static void* sbrk(long size)
{
    ...
    if (!g_last) {
        if (!region_list_append(&g_last, 0, 0)) goto sbrk_exit;
    }
    /* Assert invariants */
    assert(g_last);
    ...
    if (size >= 0) {
        ...
        if (to_commit > 0) {
            ...
            if (to_reserve > 0) {
                ...
                long remaining_commit_size =
                    (char*)g_last->top_reserved - (char*)g_last->top_committed;
                ...
                }
                {
                    ...
                    do {
                        ...
                        if (!base_reserved) {
                            int rc = GetLastError();
                            if (rc != ERROR_INVALID_ADDRESS) goto sbrk_exit;
                        }
            ...
            assert(0 < commit_size && commit_size % g_pagesize == 0);
            {
                /* Commit this */
                void* base_committed =
                    VirtualAlloc(g_last->top_committed, commit_size, MEM_COMMIT,
                                 PAGE_READWRITE);
                /* Check returned pointer for consistency */
                if (base_committed != g_last->top_committed) goto sbrk_exit;
                /* Assert postconditions */
                assert((unsigned)base_committed % g_pagesize == 0);
#ifdef TRACE
        ...
        while ((char*)g_last->top_allocated - deallocate_size <
            ...
            if (!region_list_remove(&g_last)) goto sbrk_exit;
        }
        {
                  ...
                  long arg)
{
    ...
    if (!g_regionsize) g_regionsize = getregionsize();
    /* Assert preconditions */
...
static long munmap(void* ptr, long size)
{
    ...
    assert(size % g_pagesize == 0);
    /* Free this */
    if (!VirtualFree(ptr, 0, MEM_RELEASE)) goto munmap_exit;
    rc = 0;
#ifdef TRACE
    printf("Release %p %d\n", ptr, size);
#endif
...
static int cpuinfo(int whole, CHUNK_SIZE_T* kernel, CHUNK_SIZE_T* user)
{
    if (whole) {
        ...
        int rc = GetProcessTimes(GetCurrentProcess(), (FILETIME*)&creation64,
                                 (FILETIME*)&exit64, (FILETIME*)&kernel64,
                                 (FILETIME*)&user64);
        if (!rc) {
            *kernel = 0;
            *user = 0;
        ...
        *user = (CHUNK_SIZE_T)(user64 / 10000);
        ...
        __int64 creation64, exit64, kernel64, user64;
        int rc = GetThreadTimes(GetCurrentThread(), (FILETIME*)&creation64,
                                (FILETIME*)&exit64, (FILETIME*)&kernel64,
                                (FILETIME*)&user64);
        if (!rc) {
            *kernel = 0;
