...
ApplySystem::ApplySystem() {
  ...
  LOG_DEBUG("read first page of log file.\n");
...
ApplySystem::~ApplySystem() {
  ...
  munmap(page_buf_, DATA_PAGE_SIZE);
  page_buf_ = nullptr;
}
...
bool ApplySystem::PopulateHashMap() {
  ...
  for (; next_fetch_page_id_ < end_page_id; ++next_fetch_page_id_) {
    ...
    // 读一个Page放到buf里面去
    flash_read(next_fetch_page_id_, 1, page_buf_);

    for (int block = 0; block < static_cast<int>((DATA_PAGE_SIZE / LOG_BLOCK_SIZE)); ++block) {
      if (next_fetch_page_id_ == LOG_PARTITION_START_LPA && block < 4) continue; // 跳过前面4个block
      auto hdr_no = ~LOG_BLOCK_FLUSH_BIT_MASK & mach_read_from_4(page_buf_ + block * LOG_BLOCK_SIZE);
      auto data_len = mach_read_from_2(page_buf_ + block * LOG_BLOCK_SIZE + LOG_BLOCK_HDR_DATA_LEN);
      auto first_rec = mach_read_from_2(page_buf_ + block * LOG_BLOCK_SIZE + LOG_BLOCK_FIRST_REC_GROUP);
      auto checkpoint_no = mach_read_from_4(page_buf_ + block * LOG_BLOCK_SIZE + LOG_BLOCK_CHECKPOINT_NO);
      auto checksum = mach_read_from_4(page_buf_ + block * LOG_BLOCK_SIZE + LOG_BLOCK_CHECKSUM);
      ...
      memcpy(parse_buf_ptr_ + parse_buf_content_size_,
             page_buf_ + block * LOG_BLOCK_SIZE + LOG_BLOCK_HDR_SIZE, len);
      parse_buf_content_size_ += len;
      read_blocks++;
    }
  }

  LOG_DEBUG("read %d blocks log content from log file to parse buffer.\n",
            read_blocks);
  ...
  LOG_DEBUG("parsed %d log and put them into hash table.\n",
            parsed_log_num);
...
bool ApplySystem::ApplyHashLogs() {
  ...
  LOG_DEBUG("applying logs from hash table.\n");
