...
static byte*PARSE_MLOG_REC_SEC_DELETE_MARK(byte* ptr, const byte* end_ptr) {
  ...
  ptr += 2;
  MY_ASSERT(offset <= DATA_PAGE_SIZE);
  return(ptr);
}
...
PARSE_MLOG_REC_UPDATE_IN_PLACE(byte* ptr, const byte* end_ptr) {
  ...
  MY_ASSERT(rec_offset <= DATA_PAGE_SIZE);
    ...
    const byte*	end_ptr/*!< in: buffer end */)
{
  ...
  if (end_seg_len & 0x1UL) {
    ...
    MY_ASSERT(origin_offset < DATA_PAGE_SIZE);
    mismatch_index = mach_parse_compressed(&ptr, end_ptr);
    if (ptr == nullptr) {
      return nullptr;
    }
    MY_ASSERT(mismatch_index < DATA_PAGE_SIZE);
  }
...
PARSE_DELETE_REC_LIST(
    ...
    byte*		ptr,	/*!< in: buffer */
    const byte*		end_ptr/*!< in: buffer end */) {
  MY_ASSERT(type == MLOG_LIST_END_DELETE
...
ParseOrApplyTrxUndoPageHeader(LOG_TYPE type,
  ...
  if (ptr != nullptr && page != nullptr) {
    if (type == MLOG_UNDO_HDR_CREATE) {
      ...
      MY_ASSERT(free + TRX_UNDO_LOG_XA_HDR_SIZE < DATA_PAGE_SIZE - 100);
      ...
      MY_ASSERT(mach_read_from_2(page + TRX_UNDO_PAGE_HDR
                            + TRX_UNDO_PAGE_TYPE)
           == TRX_UNDO_INSERT);
...
page_dir_get_nth_slot(const byte*	page, uint32_t n) {
  MY_ASSERT(page_dir_get_n_slots(page) > n);
    ...
    const byte*	rec)	/*!< in: the physical record */
{
  ...
  while (rec_get_n_owned_new(r) == 0) {
    ...
    MY_ASSERT(r >= page + PAGE_NEW_SUPREMUM);
    MY_ASSERT(r < page + (DATA_PAGE_SIZE - PAGE_DIR));
  }
  ...
  while (*(uint16_t *) slot != rec_offs_bytes) {
    if (slot == first_slot) {
      ...
      LOG_DEBUG("Probable data corruption on page %d. Original record on that page;", page_get_page_no(page));
      LOG_DEBUG("Cannot find the dir slot for this record on that page;\n");
    }
...
static byte* ParseRecInfoFromLog(const byte *ptr, const byte *end_ptr, RecordInfo &rec_info, bool comp) {
  ...
  if (n_uniq != n) {
    MY_ASSERT(n_uniq + DATA_ROLL_PTR <= n);
...
{
  MY_ASSERT(n_heap < 0x8000);
  MY_ASSERT(n_heap == (page_header_get_field(page, PAGE_N_HEAP) & 0x7fff) + 1);
    ...
    uint32_t slot_no)/*!< in: the directory slot */
{
  ...
  MY_ASSERT(n_owned == PAGE_DIR_SLOT_MAX_N_OWNED + 1);
  ...
  MY_ASSERT(n_owned / 2 >= PAGE_DIR_SLOT_MIN_N_OWNED);
...
//      MY_ASSERT(mach_read_from_2(page_dir_get_nth_slot(page, page_dir_get_n_slots(page) - 1) - 2) == 0);
...
page_rec_get_next_low(const byte* page, const byte*	rec) {
  ...
  if (offs >= DATA_PAGE_SIZE) {
    // Error
    LOG_DEBUG("Next record offset is nonsensical\n");
  } else if (offs == 0) {
...
static void page_dir_balance_slot(byte* page, uint32_t slot_no) {
  ...
  MY_ASSERT(n_owned == PAGE_DIR_SLOT_MIN_N_OWNED - 1);

  /* If the upper slot has the minimum value of n_owned, we will merge
  the two slots, therefore we MY_ASSERT: */
  MY_ASSERT(2 * PAGE_DIR_SLOT_MIN_N_OWNED - 1 <= PAGE_DIR_SLOT_MAX_N_OWNED);
...
page_rec_get_n_recs_before(const byte *page, const byte* rec) {
  ...
  MY_ASSERT(n >= 0);
  MY_ASSERT((uint32_t) n < DATA_PAGE_SIZE / (REC_N_NEW_EXTRA_BYTES + 1));
...
/*============================*/
  ...
  MY_ASSERT(mach_read_from_2(to_page + DATA_PAGE_SIZE - 10) == PAGE_NEW_INFIMUM);
  ...
  while (!page_cur_is_after_last(from_page, from_rec)) {
    ...
    ins_rec = page_cur_insert_rec_low(to_page,
                                      ...
                                      from_rec);
    if (!ins_rec) {
      LOG_DEBUG("page_copy_rec_list_end_no_locks error\n");
    }
...
bool ApplyCompPageReorganize(const LogEntry &log, Page *page) {
  ...
  if (data_size1 != data_size2 || max_ins_size1 != max_ins_size2) {
    LOG_DEBUG("Error, Page old data size %d new data size %d, page old max ins size %d new max ins size %d\n",
...
/*=====================*/
  ...
  MY_ASSERT(size == ULINT_UNDEFINED || size < DATA_PAGE_SIZE);

  if (rec - page == PAGE_NEW_SUPREMUM) {
    MY_ASSERT(n_recs == 0 || n_recs == ULINT_UNDEFINED);
  ...
  if ((size == ULINT_UNDEFINED) || (n_recs == ULINT_UNDEFINED)) {
    ...
    do {
      ...
      MY_ASSERT(rec2 - page + s - extra_size
            < DATA_PAGE_SIZE);
      MY_ASSERT(size + s < DATA_PAGE_SIZE);
      size += s;
      n_recs++;
  ...
  MY_ASSERT(rec_get_n_owned_new(rec2) > count);

  n_owned = rec_get_n_owned_new(rec2) - count;
  slot_index = page_dir_find_owner_slot(page, rec2);
  MY_ASSERT(slot_index > 0);
...
byte* ParseOrApplyString(byte* log_body_start_ptr, const byte* log_body_end_ptr, byte* page) {
  ...
  MY_ASSERT(offset < DATA_PAGE_SIZE);
  MY_ASSERT(len + offset <= DATA_PAGE_SIZE);
...
static byte* ParseSingleLogRecordBody(LOG_TYPE	type,
  ...
  switch (type) {
    ...
    default:
      ptr = nullptr;
      LOG_DEBUG("found unknown log type.\n");
  }
...
uint32_t ParseSingleLogRecord(LOG_TYPE &type,
  ...
  *body = nullptr;
  ...
  MY_ASSERT(type <= MLOG_BIGGEST_TYPE);
  new_ptr++;
