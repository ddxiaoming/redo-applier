...
{
  MY_ASSERT(rec);
  MY_ASSERT(mask > 0xFFUL);
  MY_ASSERT(mask <= 0xFFFFUL);
  MY_ASSERT((mask >> shift) & 1);
  MY_ASSERT(0 == ((mask >> shift) & ((mask >> shift) + 1)));
  MY_ASSERT(((mask >> shift) << shift) == mask);
  MY_ASSERT(((val << shift) & mask) == (val << shift));
...
uint32_t rec_get_info_bits(const byte* rec, bool comp) {
  ...
  MY_ASSERT(rec_info_bits_valid(val));
  return(val);
}


void rec_set_info_bits_new(byte*	rec, uint32_t bits) {
  MY_ASSERT(rec_info_bits_valid(bits));
...
/*=======================*/
    ...
    uint32_t n)	/*!< in: field index */
{
  MY_ASSERT(rec_get_1byte_offs_flag(rec));
    ...
    uint32_t len)	/*!< in: length of the data or UNIV_SQL_NULL */
{
  ...
  if (len2 == UNIV_SQL_NULL) {
    ...
    MY_ASSERT(len == rec_get_nth_field_size(rec, n));
  } else {
    MY_ASSERT(len2 == len);
  }
